...
// Include the Rust code generated by prost-build from solana.proto.
// This makes messages like `crate::schemas::solana::BlockMessage` available.
pub mod schemas {
    // The module name `solana` here corresponds to the `package solana;`
    // declaration in the `solana.proto` file.
...
pub async fn init_with_config(config: InitConfig) -> SdkResult<()> {
    ...
    // The configuration uses `EnvFilter` for dynamic log levels and outputs in JSON format.
    tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(&config.log_filter))
        .json() // Configure for structured JSON logging output
